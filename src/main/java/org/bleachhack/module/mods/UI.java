...
public class UI extends Module {
    ...
    private Text vaultText = Text.empty();
    ...
    public UI() {
        super("UI", KEY_UNBOUND, ModuleCategory.RENDER, true, "Shows stuff onscreen.",
                ...
                new SettingToggle("Vault Opening", true).withDesc("Shows the vault opening animation."));                             // 12
        ...
        // Vault Opening
        container.windows.put("auto-vault",
                new UIWindow(new Position("r", 1, "t", 0), container, () -> getSetting(12).asToggle().getState(),
                        () -> new int[]{mc.textRenderer.getWidth(vaultText) + 2, 10},
                        (ms, x, y) -> mc.textRenderer.drawWithShadow(ms, vaultText, x + 1, y + 1, 0xa0a0a0))
        );
    }
...
//        }
        ...
        int vaultTimer = AutoVault.timer;
        int vaultDelay = AutoVault.delay;

        int totalSeconds = vaultDelay * 20;
        int remainingTicks = totalSeconds - vaultTimer;
        if (remainingTicks < 0) {
            remainingTicks = 0;
        }
        ...
        vaultText = Text.literal("Auto Vault: ")
