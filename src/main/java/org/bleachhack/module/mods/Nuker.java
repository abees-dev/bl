...
public class Nuker extends Module {
  ...
  public Nuker() {
    super("Nuker", KEY_UNBOUND, ModuleCategory.WORLD, "Breaks blocks around you.",
        ...
        new SettingSlider("Multi", 1, 10, 2, 0)
            .withDesc("How many blocks to mine at once if Multi/SurvMulti mode is selected."),
        ...
        new SettingSlider("Range", 1, 20, 4.2, 1).withDesc("Mining range."),
        new SettingMode("Sort", "Closest", "Furthest", "Softest", "Hardest", "None")
            .withDesc("Which order to mine blocks in."),
  ...
  public void onTick(EventTick event) {
    ...
    for (int x = MathHelper.ceil(range); x >= MathHelper.floor(-range); x--) {
      for (int y = MathHelper.ceil(range); y >= (getSetting(8).asToggle().getState()
          ? -mc.player.getEyeHeight(mc.player.getPose()) + 0.2
          : MathHelper.floor(-range)); y--) {
        for (int z = MathHelper.ceil(range); z >= MathHelper.floor(-range); z--) {
          ...
          double distTo = getSetting(3).asMode().getMode() == 0
              ? MathHelper.absMax(
                  MathHelper.absMax(mc.player.getX() - (pos.getX() + 0.5), mc.player.getEyeY() - (pos.getY() + 0.5)),
                  mc.player.getZ() - (pos.getZ() + 0.5))
              : mc.player.getPos().distanceTo(Vec3d.ofCenter(pos));

          BlockState state = mc.world.getBlockState(pos);
          if (distTo - 0.5 > getSetting(4).asSlider().getValue() || state.isAir()
    ...
    for (ImmutablePair<BlockPos, Pair<Vec3d, Direction>> pos : blocks) {
      float breakingDelta = mc.world.getBlockState(pos.getKey()).calcBlockBreakingDelta(mc.player, mc.world,
          pos.getKey());
  ...
  private Comparator<BlockPos> getBlockOrderComparator() {
    ...
    Comparator<BlockPos> keepBlockUnderComparator = Comparator
        .comparing(BlockPos.ofFloored(mc.player.getPos().add(0, -0.8, 0))::equals);

    Comparator<BlockPos> distComparator = Comparator
        .comparingDouble(b -> mc.player.getEyePos().distanceTo(Vec3d.ofCenter(b)));
    Comparator<BlockPos> hardnessComparator = Comparator
        .comparing(b -> mc.world.getBlockState(b).getHardness(mc.world, b));
